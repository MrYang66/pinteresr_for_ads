...
class PicSpider(object):
    ...
    def is_visible(self, locator, timeout=10):
        try:
            ui.WebDriverWait(self.broser, timeout).until(
    ...
    def add_pic_hash(self, data):
        result = self.redis.sadd('hash_pic', data)  # 注意是 保存set的方式
            ...
            if stop_length:
                if stop_length - step_length < 0:
                    browser.execute_script(
                        "window.scrollBy(0,{})".format(stop_length))
                    break
    ...
    def run(self):
        ...
        url = 'http://mall.gxhyapp.com/market/web/Screening?marketCode=gz&typeid=7'
        self.broser.get(url)
        input('请选择品牌，按回车键继续：')
        while True:
            if self.is_visible('//div[@class="trends_rows"]') is True:
                elements = self.broser.find_elements_by_xpath(
                    '//div[@class="trends_rows"]')
                for element in elements:
                    img_url = element.find_element_by_xpath(
                        './/img').get_attribute('src')
                    supplier_name = element.find_element_by_xpath(
                        './/div[3]/div[1]').text
                    re_data = re.search(
                        r'http://product.aliyizhan.com/person/(.*)/(.*)/(\d+).jpg', img_url)
                    if re_data:
                        ...
                        if hash_existence is False:
                            ...
                            if self.add_good(hash2, self.brand, brand_id, 0) is True:
                                for i in range(9):
                                    hash_url = f'http://product.aliyizhan.com/person/{hash1}/{hash2}/{i}.jpg'
                                    self.add_good_detail(
                                        self.good_id, hash_url, 0, i)

                                # with closing(requests.get(hash_url, stream=True, timeout=5)) as r:
                                #     if r.status_code == 200:
                                #         chunk_size = 1024
                                #         content_size = int(r.headers['content-length'])
                                #         with open(f'./{supplier_name}/{time.strftime("%F")}/{hash2}/{i}.jpg', "wb+") as f:
                                #             n = 1
                                #             with tqdm(r.iter_content(chunk_size=chunk_size)) as t:
                                #                 for chunk in t:
                                #                     loaded = n * 1024.0 / content_size
                                #                     # 设置进度条左边显示的信息
                                #                     t.set_description("正在下载 {}".format(f'{hash2}/{i}.jpg'))
                                #                     f.write(chunk)
                                #                     # 设置进度条右边显示的信息
                ...
                if self.is_visible('//*[@id="app"]/div/div/div[3]/div[2]/div[2]/div/ul/li[12]') is True and self.is_visible('//*[@id="app"]/div/div/div[3]/div[2]/div[2]/div/button[2]') is True:
                    self.get_page += 1
                    if self.get_page == 30:
                        self.broser.quit()
                        break
                        ...
                        self.broser.find_element_by_xpath(
                            '//*[@id="app"]/div/div/div[3]/div[2]/div[2]/div/button[2]').click()
                else:
                    self.roll_window_to_bottom(self.broser, stop_length=2000)
            else:
